...
public class TinyEngine {
    ...
    private static class Display extends JPanel implements MouseListener {
        ...
        private <T extends Bounded> void propagate(final Class<T> listenerClass, Consumer<T> action) {
            things.stream()
                    .filter(t -> listenerClass.isAssignableFrom(t.getClass()))
                    .map(t -> (T) t)
                    .filter(t ->
                        t.getCollidingZone() != null && t.getCollidingZone().getBounds().contains(mousePosition.x, mousePosition.y))
